const photoIMG = require('./photo.png');
const igtvIMG = require('./igtv-icon.png');
const messageIMG = require('./send-message.png');
const myStoryIMG = require('./my-stories.png');
const avatarIMG = require('./avatar.png');
const moreIcon = require('./more_Icon.png');
const postIMG = require('./post.png')
const userIconIMG = require('./user-icon.png');
const heartIMG = require('./heart.png');
const commentIMG = require('./comment.png');
const saveIMG = require('./save.png');
const homeIMG = require('./home.png');
const searchIMG = require('./search.png');
const createPostIMG = require('./create-post.png');
const actionHeartIMG = require('./hear.png');
const cmttIMG = require('./comment-ttt.png');
const heartttIMG = require('./heart-tt.png');

export { photoIMG, igtvIMG, messageIMG, myStoryIMG, avatarIMG, moreIcon, postIMG, userIconIMG, heartIMG, commentIMG, saveIMG, homeIMG, searchIMG, createPostIMG, actionHeartIMG, cmttIMG, heartttIMG };
